...
#   arm64 build : native; PYO3_PYTHON is used if set, otherwise uv python find.
#   x86_64 build: cross-compiled from arm64 host. PYO3_CROSS_PYTHON_VERSION tells
...
#                 can find libpythonX.Y (works for both the Python.org universal2
...
# This ensures the linker's -lpythonX.Y search has a valid -L path during
...
# PYO3_CROSS_LIB_DIR: tell the linker where libpythonX.Y lives.
# The Python.org universal2 framework dylib at $PY_PREFIX/lib/libpythonX.Y.dylib
# contains both arm64 and x86_64 slices, so the linker picks the right one.
PYO3_CROSS_PYTHON_VERSION="$PY_VERSION" PYO3_CROSS_LIB_DIR="${PY_LIB_DIR}" \
